...
19. **Rich Messages are a separate send surface** — use `rich`/`markdownTable` for the Markdown lane, `blocks`/`table` for native structured blocks, or `@gramio/jsx/rich` for rich JSX. Rich values route to `sendRichMessage`, not `sendMessage`. `streamRichMessage` is Markdown-chunk based; send native blocks directly with `ctx.send` / `ctx.sendRichMessage`. Uploads work recursively in `rich_message.blocks` / `.media`, but direct uploads are forbidden in `sendRichMessageDraft`. Handle user cancellation with `bot.on("stopped_message_generation", ...)`. See [rich-messages](references/rich-messages.md).
...
| Rich Messages | Markdown/structured block helpers, rich JSX, recursive uploads, drafts, stop-generation updates | [rich-messages](references/rich-messages.md) |
